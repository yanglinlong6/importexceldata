...
if __name__ == '__main__':
    ...
    cursor = conn.cursor()
    sql0 = '''
    DELETE FROM yll_copy_log_0401_03;
    '''
    cursor.execute(sql0)
    print('清理日志记录数据库')

    sql00 = '''
    SELECT DISTINCT DATE_FORMAT(DATE_SUB(`settle_date`, INTERVAL 1 MONTH), '%Y%m'), customer_type FROM d_settle_doc_big_customer_yll0401_03 
    ...
    if dateType[1] in customerTypeList:
        ...
        # sql01 = '''
        # select a.sn fromd_settle_doc_big_customer_yll0401_03 a left join shwu_settle_detail b on a.sn = b.sn where b.sn is null
        # and a.customer_type = %s and DATE_FORMAT(DATE_SUB(a.settle_date, INTERVAL 1 MONTH),'%%Y%%m') = %s;
        # '''

        sql01 = '''
                select id, sn from d_settle_doc_big_customer_yll0401_03 WHERE customer_type = %s and DATE_FORMAT(DATE_SUB(settle_date, INTERVAL 1 MONTH),'%%Y%%m') = %s;
                '''
        ...
        # sql02 = '''
        # select b.sn from shwu_settle_detail b left joind_settle_doc_big_customer_yll0401_03 a on a.sn = b.sn where a.sn is null
        ...
        if shwuSnList and bigCustomerSnList:
            for indexNum, shwuSn in enumerate(shwuSnList):
                ...
                if indexNum < len(bigCustomerSnList):
                    ...
                    sql03 = " UPDATE d_settle_doc_big_customer_yll0401_03 SET `sn` = %s , `del_flag` = 3 WHERE `id` = %s;"
                    ...
                    # 提交事务
                    conn.commit()
                    sql04 = "INSERT INTO yll_copy_log_0401_03(source_sn, new_sn) VALUES( %s, %s);"
                    cursor.execute(sql04, [bigCustomerSnList[indexNum][1], shwuSnList[indexNum][0]])
                    # 提交事务
                    conn.commit()
