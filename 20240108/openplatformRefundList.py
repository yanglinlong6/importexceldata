...
if __name__ == '__main__':
    ...
    sql0 = '''
    select device_type from t_settle_order_202307to12 group by device_type;
    '''
    cursor.execute(sql0)
    ...
    for deviceType in deviceTypeList:
        sql00 = '''
        SELECT settle_date ,customer_id ,settle_count ,device_type ,specific_version ,id FROM t_settle_order_202307to12 where device_type = %s and settle_count < 0 order by settle_date;   
        '''
        print("执行的SQL==", sql00)
        ...
        for openPlatformSettle in openPlatformSettleList:
            sql01 = '''
                select device_sn from t_platform_device_yll_20240108 where device_type = %s and create_time < %s and version = %s and status = 1  and customer_id = %s order by create_time desc ;
                '''
            print("执行的SQL==", sql01)
            ...
            for index in range(abs(openPlatformSettle[2])):
                print('查出所有可以退货的数据==', len(deviceSnList), '应该退货的数量', abs(openPlatformSettle[2]))
                if index < len(deviceSnList):
                    sql02 = '''
                            update t_platform_device_yll_20240108 set status = 0 , remark = concat(remark,';退货的ID:',%s) where device_sn = %s ;
                        '''
                    print("执行的SQL==", sql02)
                    cursor.execute(sql02, [openPlatformSettle[5],deviceSnList[index][0]])
