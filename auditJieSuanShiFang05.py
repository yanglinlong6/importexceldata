...
if __name__ == '__main__':
    conn = pymysql.connect(host='192.168.1.39',
                           ...
                           autocommit='True')
    cursor = conn.cursor()
    sql01 = "SELECT new_sn ,count(1) as num from d_bigcopy_change group by new_sn HAVING num >1;"
    ...
    for activeDate in activeDateList:
        for customerName in customerNameList:
            ...
            sql02 = "SELECT sn ,activeDate FROM d_device_activate_detail WHERE LEFT(activeDate,7) <= '2020-12' and LEFT(activeDate,7) >= %s and customer_name = '上海宝信实嘉汽车销售有限公司天津武清分公司' and sn not in (SELECT sn FROM d_settle_doc_big_customer_copy) group by sn order by activeDate;"
