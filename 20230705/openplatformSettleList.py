...
if __name__ == '__main__':
    ...
    sql = '''
    TRUNCATE TABLE t_platform_device_yll_20230705;
    '''
    cursor.execute(sql)
    print('清理临时操作记录数据库')
    ...
    for deviceType in deviceTypeList:
        sql00 = '''
        SELECT settle_date ,customer_id ,settle_count ,device_type ,specific_version ,id  FROM t_settle_order_202301to06 where device_type = %s and settle_count >= 0;   
        '''
        ...
        for openPlatformSettle in openPlatformSettleList:
            ...
            for index in range(openPlatformSettle[2]):
                ...
                sql02 = '''
                      INSERT INTO t_platform_device_yll_20230705
                ...
                cursor.execute(sql02, [str(maxFragDeviceSn), openPlatformSettle[3], openPlatformSettle[4],
                                       openPlatformSettle[1], None, None, 1, openPlatformSettle[0], None, 0, 0, openPlatformSettle[5]])
                num = num + 1
