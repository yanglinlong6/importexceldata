...
if __name__ == '__main__':
    ...
    for activeDate in activeDateList:
        for customerName in customerNameList:
            ...
            sql02 = "SELECT sn ,activeDate FROM d_device_activate_detail WHERE LEFT(activeDate,7) >= '2017-11' and LEFT(activeDate,7) <= %s and customer_name = %s and sn not in (SELECT sn FROM d_settle_doc_big_customer_copy) group by sn order by activeDate desc ;"
            ...
            sql05 = "SELECT sn ,activeDate FROM d_device_activate_detail WHERE LEFT(activeDate,7) <= '2020-12' and LEFT(activeDate,7) >= %s and customer_name = %s and sn not in (SELECT sn FROM d_settle_doc_big_customer_copy) group by sn order by activeDate;"
            cursor.execute(sql05, [activeDate[0], customerName])
            snBehindList = cursor.fetchall()
            print(snBehindList.__len__())
            ...
            sql06 = "UPDATE `d_bigcopy_change` SET `new_sn` = %s, `new_active_date` = %s ,`active_flag` = 1 WHERE `id` = %s;"
                ...
                if indexNum < len(snAndActiveDateList):
                    while flagOne:
                        if indexNum < len(snAndActiveDateList):
                            ...
                            if indexNumTwo < len(snBehindList):
                                while flagTwo:
                                    if indexNumTwo < len(snBehindList):
                                        if snBehindList[indexNumTwo][0] not in snQuChongList:
                                            snQuChongList.append(snBehindList[indexNumTwo][0])
                                            cursor.execute(sql06,
                                                           [snBehindList[indexNumTwo][0], snBehindList[indexNumTwo][1],
                                                            tiHuId])
                                ...
                                print(activeDate[0], customerName, "依旧缺少的数",
                                      len(tiHuIdList) - len(snAndActiveDateList) - len(snBehindList))
                        ...
                        while flag:
                            if indexNumTwo < len(snBehindList):
                                if snBehindList[indexNumTwo][0] not in snQuChongList:
                                    snQuChongList.append(snBehindList[indexNumTwo][0])
                                    cursor.execute(sql06,
                                                   [snBehindList[indexNumTwo][0], snBehindList[indexNumTwo][1], tiHuId])
                                    indexNumTwo = indexNumTwo + 1
                                    print('弥补缺失成功')
                                    flag = False
                                else:
